var transkey_url = '/inc/js/transkey';
var transkey_surl ='/transkeyServlet2';
...
var licenseType = "";
var licExpiredDate = "";
var usePressImg = true; //default : false
var pressImgTime = 1000;
//config
...
//토스 보안키패드 매수, 매도시 4자리 비밀번호 입력완료시 실행 callBack
var mtkCloseCallback;

//토스 보안키패드 매수, 매도시 4자리 비밀번호 입력할때 마다 실행 callBack
var mtkStartCallback;

//토스 보안키패드 매수, 매도시 4자리 비밀번호 입력지울때 마다 실행 callBack
var mtkDeleteCallback;
...
mTranskeyObj.prototype.setWidth = function(width){
	if(width>=600&&this.clientHeight>=600&&!mtk.horizontal){
		if(widthRatio != 1)
			this.width = width*widthRatio-30;
		else
			this.width=600;
	} else if(width>=360)
		this.width=360;
...
function mTranskey(){
	 ...
	 this.setPosition = function(){
		 ...
		 if(isBottom == undefined||isBottom=="false"){
			 if(xy == undefined){
			 	...
			 	var top = document.documentElement.clientHeight-getComputedStyle(div).height.replace("px","")-15;  //20200713 바닥 마진 -15 바꿈 기존 -2였음
				var left = 0;
				if(transkey_divType == 0) {
					 var point = getOffsetPoint(inputObj);
					 var left = this.clientWidth/2 - this.now.width/2;
	...
	this.setKeyIndex = function(inputObj) {
	 ...
	 this.setHiddenField = function(inputObj, ExE2E){
		...
		/* 토스뱅크에서 사용할 input 추가 custom */
		var kakaoDecName = document.createElement("input");
		kakaoDecName.setAttribute("type", "hidden");
		kakaoDecName.setAttribute("id", "transkey_accName");
		kakaoDecName.setAttribute("name", "transkey_accName");
		kakaoDecName.setAttribute("value", inputObj.id);

		var kakaoDecId = document.createElement("input");
		kakaoDecId.setAttribute("type", "hidden");
		kakaoDecId.setAttribute("id", "transkey_accId");
		kakaoDecId.setAttribute("name", "transkey_accId");
		kakaoDecId.setAttribute("value", inputObj.id);

		obj.appendChild(hidden);
		obj.appendChild(hmac);
		obj.appendChild(kakaoDecName);
		obj.appendChild(kakaoDecId);
		if(ExE2E!=null){
	...
	function qwertyMobileLayoutForDIV(id, cssText){
		var layout = '<div id="mtk_disp" class="dv_'+cssText+'_disp" role="button" tabindex="0" aria-label="가상키보드 영역 입니다">';
	...
	//20200701 폰트 위치 관련 수정
	//20200713 폰트 위치 관련 세로 +4씩 수정함
	function numberMobileNDLayoutForDIV(id, cssText){
		...
		layout += '<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 0);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-48px -17px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 1);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-173px -17px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 2);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-296px -17px;"></div></div>'+
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 3);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-48px -79px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 4);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-173px -79px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 5);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-296px -79px;"></div></div>'+
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 6);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-48px -141px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 7);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-173px -141px;"></div></div>'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 8);" role="button" tabindex="0"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-296px -141px;"></div></div>'+
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height">'+
	    //기능키 변경 20200413
	    ...
	    '<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div_a dv_'+cssText+'_div2_Height" role="button" tabindex="0" aria-label="전체 삭제"><div class="dv_'+cssText+'_div3_3 dv_'+cssText+'_div3_3_nTop" style="background-position:-47px -202px;"></div></div>'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.start(event, this, 9);" role="button" tabindex="0" aria-label="전체 삭제"><div class="dv_'+cssText+'_div2_3 dv_'+cssText+'_div2_2_nTop" style="background-position:-172px -204px;"></div></div>'+
		...
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div_b dv_'+cssText+'_end dv_'+cssText+'_div2_Height"  '+startEvent+'="mtk.del(event, this);" role="button" tabindex="0" aria-label="삭제"><div class="dv_'+cssText+'_div3_4 dv_'+cssText+'_div3_3_nTop" style="background-position:-292px -196px;"></div></div>'+
	'</div>';
		if(showLicense&&licenseType!="")
			layout += '<div class="dv_'+cssText+'_licenseType" style="position : absolute;margin: 5px; color : red;">' + licenseType + " (" + licExpiredDate + ")" + '</div>';
...
mTranskey.prototype.setKeyboard = function(inputObj){
	...
	document.getElementById("layer-pop-password").appendChild(div);
...
mTranskey.prototype.makePressImg = function(){
	...
	pressImg.setAttribute("style", "position: absolute; top: 50%; left: 50%; height:80%; transform: translate(-50%, -50%);");
	return pressImg;
};
...
mTranskey.prototype.fakeKeyPress = function(fakeKey){
	try{
		...
		fakeKey.style.backgroundColor="#FAFAFA";
		if(usePressImg)
...
mTranskey.prototype.startCallBack = function(){
	// 현재는 토뱅에서 매수, 매도시
	// 계좌 4자리 입력시마다 실행할 콜백
	if(typeof mtkStartCallback == 'function')
		mtkStartCallback();
};
...
mTranskey.prototype.closeCallBack = function(){
	// 현재는 토뱅에서 매수, 매도시
	// 계좌 4자리 입력 완료시 실행할 콜백
	if(typeof mtkCloseCallback == 'function')
		mtkCloseCallback();
};

mTranskey.prototype.delCallBack = function(){
	if(typeof mtkDeleteCallback == 'function')
		mtkDeleteCallback();
};
...
mTranskey.prototype.reSizeForType1 = function(transkeyObj){
	...
	if(transkeyObj.keyboardType=="numberMobile"){
		...
		for(var i=1; 5>i; i++){
			if(i==4){
				//20200701 각 버튼 크기 관련 수정
				transkeyObj.div.childNodes[i].childNodes[0].style.width=ddivWidth+edgePx[0]+1+"px";
				transkeyObj.div.childNodes[i].childNodes[1].style.width=ddivWidth+edgePx[1]+1+"px";
				transkeyObj.div.childNodes[i].childNodes[2].style.width=ddivWidth+ddivWidth+1+edgePx[2]+edgePx[3]+2+"px";
				if(useNoDummy)
					transkeyObj.div.childNodes[i].childNodes[2].style.width=ddivWidth+edgePx[2]+2+"px";
			}else{
				for(var j=0; k>j; j++){
					if(j==2){
						transkeyObj.div.childNodes[i].childNodes[j].style.width=ddivWidth+edgePx[j]+2+"px";
					}else{
						transkeyObj.div.childNodes[i].childNodes[j].style.width=ddivWidth+edgePx[j]+1+"px";
					}
				}
			}
